...
/*********************************************************/
uint16_t colorLUT[256];  // lookup table
...
void initColorLUT() {
  for (int val = 0; val < 256; val++) {
    ...
    colorLUT[val] = rgb565(r,g,b);
  }
}


//******************************************* draws the waterfall, is time critical

void display_fft_graf(uint16_t freq) {
  static uint16_t freq_old = 7080;
  int16_t freq_change = (int16_t)freq - (int16_t)freq_old;

  uint16_t extra_color = TFT_DARKGREY;

  // Plot waterfall
  for (int y = 0; y < GRAPH_NUM_LINES; y ++) {
    // Erase line
    tft.drawFastHLine(0, (GRAPH_NUM_LINES + Y_MIN_DRAW - y),
                      GRAPH_NUM_COLS, TFT_DARKBLUE);

    // Plot line
    for (int x = 0; x < GRAPH_NUM_COLS; x++) {
      uint16_t val =  vet_graf_fft[y][x] * 30;

     val= constrain(val, 0, 255); 


      uint16_t re = colorLUT[val];

      if (val) {
        if ((x >= triang_x_min) && (x <= triang_x_max))
          tft.drawPixel(x, (GRAPH_NUM_LINES + Y_MIN_DRAW - y), re | extra_color);
        else
          tft.drawPixel(x, (GRAPH_NUM_LINES + Y_MIN_DRAW - y), re);
      } else {
        if ((x >= triang_x_min) && (x <= triang_x_max))
          tft.drawPixel(x, (GRAPH_NUM_LINES + Y_MIN_DRAW - y),
                        TFT_BACKGROUND | extra_color);
      }
    }
  }
...
initColorLUT(); // init color lookup table for waterfall
